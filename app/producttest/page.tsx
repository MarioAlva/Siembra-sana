				...
				<section className="flex w-full [&>div]:flex [&>div]:flex-col [&>div]:w-1/2 justify-around mt-12 h-auto">
					...
					<div className='justify-between'>
						<div>
							...
							<div className='flex md:items-center items-start md:text-xl sm:text-xs text-[10px] md:mt-2 md-1'>
								<span className='text-neutral-500'>Empaquetado: </span>
								<span className='text-neutral-700'>&nbsp;{product.pack[packIndex].packing}</span>
							</div>
							<div className='flex'>
							{product.pack.length > 0 && product.pack.map((pack : any, index : number) => (
								<div key={index} className='mx-1 flex justify-center items-center rounded-full bg-green-300 hover:bg-green-400 cursor-pointer md:px-3 px-[6px] md:py-1 py-[2px]' onClick={() => setPackIndex(index)}>
									<span className='text-neutral-700 text-center text-[10px] md:text-base'>{pack.weight}g</span>
								</div>
							))}
							</div>
						</div>
						<div>
							<div className='flex items-baseline justify-center'>
								<span className='md:text-3xl text-base font-bold'>{product.pack[packIndex].price}€</span>
						...
						<div className='flex flex-col md:items-end items-start'>
							<div className='flex md:items-center items-start md:text-base text-[10px]'>
								<span className='text-neutral-500'>Categoría: </span>
								<span className='text-neutral-700'>{product.genre.join(', ')}</span>
							</div>
